...
import { YarnTreeNode } from '../../dependencyTree/dependenciesRoot/yarnTree';
...
export class YarnUtils {
    ...
    public static isVersionSupported(parent: DependencyTreeNode, logManager: LogManager, workspaceFolder: string): boolean {
        try {
            ...
            if (yarnSemver.compare('2.0.0') >= 0) {
                ...
                new DependencyTreeNode(generalInfo, vscode.TreeItemCollapsibleState.None, parent);
                return false;
            }
        } catch (error) {
