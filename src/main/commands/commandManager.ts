        ...
        if (node instanceof CodeIssueTreeNode) {
            try {
                ...
                if (issue_details["cve"] && issue_details["component"] && issue_details["version"]) {
                    ...
                    const upgrade_fix = "1) Upgrade the version of the dependency by consulting `get_curation_package_status` tool to check whether an alternative version will be acceptable.";
                    prompt = `${base_propmpt}\n${upgrade_fix}`
                    if (issue_details["cve"]["applicableData"] && issue_details["extendedInformation"] && issue_details["extendedInformation"]["remediation"]) {
                        const remediation = `2) Remediate the use of the dependency in the source code according to the following instructions: \`${issue_details["extendedInformation"]["remediation"]}\`. Use the following vulnerable API use location evidence in the source code: \`${JSON.stringify(issue_details["cve"]["applicableData"]["evidence"])}\``;
                        prompt = `${prompt}\n${remediation}`;
                    }
                } else {

                prompt = `Here is a vulnerability details page. Please suggest a fix for the vulnerability: ${JSON.stringify(
                    issue_details
                )}`;
            }
