            ...
            try {
                ...
                if (issue_details["cve"] && issue_details["component"] && issue_details["version"] && issue_details["infectedVersion"]) {
                    ...
                    if (issue_details["cve"]["applicableData"] && issue_details["extendedInformation"] && issue_details["extendedInformation"]["remediation"]) {
                        const remediation = `2) Remediate the use of the dependency in the source code according to the following instructions: \`${issue_details["extendedInformation"]["remediation"]}\`. Use the following vulnerable API use location evidence in the source code (only fix the source code related to the following location!): \`${JSON.stringify(issue_details["cve"]["applicableData"]["evidence"])}\``;
                        prompt = `${prompt}\n${remediation}`;
                    }
                } else {
