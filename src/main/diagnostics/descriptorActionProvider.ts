...
export class DescriptorActionProvider extends AbstractFileActionProvider {
    ...
    public async updateDiagnostics(document: vscode.TextDocument): Promise<void> {
        ...
        if (fileIssues instanceof DescriptorTreeNode) {
            ...
            fileIssues.dependenciesWithIssue.forEach(dependencyWithIssue => {
                dependencyWithIssue.impactedTree?.children
                    ?.map(impact => impact.name /*.substring(0,impact.name.lastIndexOf(":"))*/)
                    .forEach(directDependencyName => {
                        let position: vscode.Position[] = this.getDependencyPosition(
                            documentPath,
                            document,
                        ...
                        if (position.length === 0) {
                            this._treesManager.logManager.logMessage(
                                "<ASSAFA> can't find positions for dependency '" + directDependencyName + "'",
                                'DEBUG'
                            );
                            return;
                        }
                        ...
                        this._treesManager.logManager.logMessage(
                            "<ASSAFA> creating diagnostics for dependency '" + directDependencyName + "' (position.length = " + position.length + ')',
                            'DEBUG'
                        );
                        diagnostics.push(
    ...
    private getDependencyPosition(
        ...
        switch (packeType) {
            case PackageType.Go:
                return GoUtils.getDependencyPosition(document, searchParameter, FocusType.Dependency);
            case PackageType.Maven:
                return MavenUtils.getDependencyPos(document, undefined, FocusType.Dependency); // TODO: FIX THIS
            case PackageType.Npm:
                return NpmUtils.getDependencyPosition(document, searchParameter, FocusType.Dependency);
            case PackageType.Python:
                return PypiUtils.getDependencyPosition(document, searchParameter);
            case PackageType.Yarn:
                return YarnUtils.getDependencyPosition(document, searchParameter);
            default:
                this._treesManager.logManager.logMessage(
                    "<ASSAFA> no positions for dependency '" + dependencyId + "' in descritor '" + documentPath + "'",
                    'DEBUG'
                );
                return [];
        }
