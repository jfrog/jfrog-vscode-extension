...
export class GoTreeNode extends RootNode {
    ...
    constructor(tmpWorkspaceFolder: string, private _treesManager: TreesManager, parent?: DependencyTreeNode) {
    ...
    private buildDependenciesMapAndDirectDeps(goModGraph: PackageDependencyPair[], goList: string[], goVersion: SemVer): Map<string, string[]> {
        ...
        directDependenciesGeneralInfos.forEach(generalInfo => {
            this.addChild(new DependencyTreeNode(generalInfo, this.getTreeCollapsibleState(dependenciesMap, generalInfo)));
        });
        // Add go version as a direct dependency to scan it as well
        this.addChild(new DependencyTreeNode(new GeneralInfo('github.com/golang/go', goVersion.format(), ['None'], '', PackageType.Go)));

        return dependenciesMap;
    }

    private populateDependenciesTree(dependenciesMap: Map<string, string[]>, dependenciesTreeNode: DependencyTreeNode) {
        ...
        childDependencies.forEach(childDependency => {
            ...
            let grandchild: DependencyTreeNode = new DependencyTreeNode(
                generalInfo,
    ...
    private hasLoop(dependenciesTreeNode: DependencyTreeNode): boolean {
        let parent: DependencyTreeNode | undefined = dependenciesTreeNode.parent;
        while (parent) {
    ...
    private addComponentToScan(dependenciesTreeNode: DependencyTreeNode) {
