...
import { PomTree } from '../../utils/dependency/pomTree';

export class MavenTreeNode extends RootNode {
    private static readonly COMPONENT_PREFIX: string = 'gav://';

    constructor(workspaceFolder: string, private _treesManager: TreesManager, parent?: DependencyTreeNode) {
     ...
     * @param quickScan - True to allow reading from scan cache.
     */
    private populateDependenciesTree(parent: DependencyTreeNode, rawDependenciesList: string[], rawDependenciesPtr: { index: number }) {
        for (; rawDependenciesPtr.index < rawDependenciesList.length; rawDependenciesPtr.index++) {
            ...
            let child: DependencyTreeNode = new DependencyTreeNode(gavGeneralInfo, treeCollapsibleState, parent);
