...
export class BuildsDataProvider implements vscode.TreeDataProvider<DependencyTreeNode> {
    private _onDidChangeTreeData: vscode.EventEmitter<DependencyTreeNode | undefined> = new vscode.EventEmitter<DependencyTreeNode | undefined>();
    readonly onDidChangeTreeData: vscode.Event<DependencyTreeNode | undefined> = this._onDidChangeTreeData.event;

    private _filteredDependenciesTree: DependencyTreeNode | undefined;
    protected _dependenciesTree!: DependencyTreeNode;
    protected _allBuildsTree!: DependencyTreeNode;
    ...
    public getChildren(element?: DependencyTreeNode): Thenable<DependencyTreeNode[]> {
        if (!this.dependenciesTree) {
        ...
        let rootChildren: DependencyTreeNode[] = this._filteredDependenciesTree
    ...
    private fillFilters(node: DependencyTreeNode) {
    ...
    public getParent(element: DependencyTreeNode): Thenable<DependencyTreeNode | undefined> {
        return Promise.resolve(element.parent);
    }

    public applyFilters(filteredDependenciesTree: DependencyTreeNode | undefined) {
        this._filteredDependenciesTree = filteredDependenciesTree;
        this.onChangeFire();
    }

    public removeNode(node: DependencyTreeNode, onChangeFire: () => void) {
    ...
    private async repopulateTree(quickScan: boolean, credentialsSet: boolean) {
        await ScanUtils.scanWithProgress(async (progress: vscode.Progress<{ message?: string; increment?: number }>, checkCanceled: () => void) => {
            this.clearAllTrees();
            if (credentialsSet) {
                let allBuilds: DependencyTreeNode = <DependencyTreeNode>this.allBuildsTree;
    ...
    public loadFirstBuild(quickScan: boolean): void {
        if (!!this._allBuildsTree && this._allBuildsTree.children.length > 0) {
            let dependencyTree: DependencyTreeNode = this._allBuildsTree.children[0];
    ...
    private clearAllTrees() {
        ...
        this._allBuildsTree = new DependencyTreeNode(generalInfo, vscode.TreeItemCollapsibleState.Expanded);
    }

    private clearDisplayedTree() {
        let generalInfo: GeneralInfo = new GeneralInfo('', '', [], '', PackageType.Unknown);
        this._dependenciesTree = new DependencyTreeNode(generalInfo, vscode.TreeItemCollapsibleState.Expanded);
    }
