...
export class CiManager {
    ...
    private root: DependencyTreeNode;

    constructor(private _treesManager: TreesManager, root?: DependencyTreeNode) {
        ...
        if (!root) {
            this.root = new DependencyTreeNode(
    ...
    public loadBuildTree(timestamp: string, buildName: string, buildNumber: string, parent: DependencyTreeNode) {
    ...
    public populateModulesDependencyTree(build: any, modulesRoot: DependencyTreeNode) {
        for (const module of build.modules) {
            ...
            if (BuildsUtils.isArrayExistsAndNotEmpty(module, CiTitleNode.ARTIFACTS_NODE)) {
                const artifactsNode: DependencyTreeNode = BuildsUtils.createArtifactsNode();
            ...
            if (BuildsUtils.isArrayExistsAndNotEmpty(module, CiTitleNode.DEPENDENCIES_NODE)) {
                const dependenciesNode: DependencyTreeNode = BuildsUtils.createDependenciesNode();
    ...
    public populateArtifacts(artifactsNode: DependencyTreeNode, module: any): void {
        for (const artifact of module.artifacts) {
            ...
            const artifactNode: DependencyTreeNode = new DependencyTreeNode(artifactGeneralInfo, vscode.TreeItemCollapsibleState.None, undefined, '');
            artifactsNode.addChild(artifactNode);
        }
    }

    public populateDependencies(dependenciesNode: DependencyTreeNode, module: any): void {
    ...
    private populateTransitiveDependencies(dependency: Dependency, parentToChildren: Map<string, Dependency[]>): DependencyTreeNode {
        ...
        let dependencyTree: DependencyTreeNode = new DependencyTreeNode(
    ...
    public populateBuildDependencyTree(response: IDetailsResponse | null, modulesTree: DependencyTreeNode): void {
        if (!response) {
    ...
    private addUnknownLicenseToMissingNode(node: DependencyTreeNode) {
        node.licenses.add({ licenseName: Constants.UNKNOWN_LICENSE } as ILicenseKey);
    }

    public populateTreeWithUnknownIssues(modulesTree: DependencyTreeNode) {
