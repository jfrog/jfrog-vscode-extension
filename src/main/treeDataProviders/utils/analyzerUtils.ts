...
export class AnalyzerUtils {
    ...
    // Use in apply on issues root, this will auto create files if not exists on apply so it does not matter what scan finish first (grpah or applicable)
    // public static populateApplicableCodeFileIssueIfNotExists(root: IssuesRootTreeNode) {
    //     let i: number = 0;
    //     return i;
    // }
}
